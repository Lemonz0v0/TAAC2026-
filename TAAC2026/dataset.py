...
# Recency bucket boundaries for sample timestamp - sequence timestamp.
# 63 edges -> 64 embedding ids including padding(0).  Keep the edge count equal
# to the previous version so time_embedding shape remains stable, while moving
# capacity away from mostly redundant 5-second splits and toward 1h~1y ranges.
# Rationale: TAAC sequence timestamps in the public demo are mostly minute-level;
# the model also has per-domain residual embeddings, so one shared boundary set
# plus domain residuals is a better bias/variance trade-off than four independent
# boundary systems.
BUCKET_BOUNDARIES = np.array([
    # sub-minute / first hour
    1, 5, 15, 30, 45, 60, 90, 120, 180, 240, 300, 420, 600,
    900, 1200, 1500, 1800, 2400, 2700, 3000, 3600,
    # same-day behavior: 1.5h -> 24h
    5400, 7200, 9000, 10800, 12600, 14400, 16200, 18000,
    21600, 28800, 36000, 43200, 54000, 64800, 75600, 86400,
    # 1.5d -> 30d
    129600, 172800, 259200, 345600, 432000, 518400, 604800,
    864000, 1209600, 1814400, 2419200, 2592000,
    # 40d -> 18mo
    3456000, 4320000, 5184000, 6048000, 7776000, 10368000,
    12960000, 15552000, 18144000, 20736000, 23328000, 25920000,
    31536000, 47304000,
], dtype=np.int64)
...
# Per-domain sequence overflow summary feature layout.  These features are
# computed from the *raw* sequence lengths before truncation, so they preserve
# information that is otherwise lost when long histories are capped to
# seq_max_lens (e.g. seq_d raw length 4000 and 512 both become len=512).
# For each domain we emit five float features:
#   0: log1p(raw_len) / log1p(max_len * 8)
#   1: log1p(max(raw_len - max_len, 0)) / log1p(max_len * 8)
#   2: 1 if raw_len > max_len else 0
#   3: kept_ratio = min(raw_len, max_len) / raw_len
#   4: overflow_ratio = max(raw_len - max_len, 0) / raw_len
SEQ_OVERFLOW_FEATURES_PER_DOMAIN = 5
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        self._seq_overflow_summary_dim = len(self.seq_domains) * SEQ_OVERFLOW_FEATURES_PER_DOMAIN
        self._buf_seq_overflow_summary = np.zeros(
            (B, self._seq_overflow_summary_dim), dtype=np.float32
        )
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        time_summary[:] = 0
        seq_overflow_summary = self._buf_seq_overflow_summary[:B]
        seq_overflow_summary[:] = 0
        ...
        for domain_idx, domain in enumerate(self.seq_domains):
            ...
            # Raw-length overflow summary before cap-induced information is lost.
            # Length mismatch rate across sequence columns was 0 in the train probe,
            # but max() keeps this robust if a future schema has partial columns.
            raw_len_f = raw_lengths.astype(np.float32)
            kept_len_f = lengths.astype(np.float32)
            overflow_f = np.maximum(raw_len_f - float(max_len), 0.0)
            nonzero_raw = raw_len_f > 0
            denom = np.log1p(float(max(max_len * 8, max_len + 1)))
            kept_ratio = np.zeros(B, dtype=np.float32)
            overflow_ratio = np.zeros(B, dtype=np.float32)
            kept_ratio[nonzero_raw] = kept_len_f[nonzero_raw] / raw_len_f[nonzero_raw]
            overflow_ratio[nonzero_raw] = overflow_f[nonzero_raw] / raw_len_f[nonzero_raw]
            ov_base = domain_idx * SEQ_OVERFLOW_FEATURES_PER_DOMAIN
            seq_overflow_summary[:, ov_base + 0] = np.clip(np.log1p(raw_len_f) / denom, 0.0, 2.0)
            seq_overflow_summary[:, ov_base + 1] = np.clip(np.log1p(overflow_f) / denom, 0.0, 2.0)
            seq_overflow_summary[:, ov_base + 2] = (raw_len_f > float(max_len)).astype(np.float32)
            seq_overflow_summary[:, ov_base + 3] = kept_ratio
            seq_overflow_summary[:, ov_base + 4] = overflow_ratio
        ...
        result['seq_overflow_summary_feats'] = torch.from_numpy(
            seq_overflow_summary.copy()
        )
        return result
