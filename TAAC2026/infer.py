...
_FALLBACK_MODEL_CFG = {
    ...
    'use_seq_overflow_summary_features': False,
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in _MODEL_CFG_KEYS:
        if key == 'num_time_buckets':
            ...
            continue
        if key == 'seq_longer_gather_side':
            if 'seq_longer_gather_side' in train_config:
                cfg[key] = train_config['seq_longer_gather_side']
            elif 'longer_gather_side' in train_config:
                cfg[key] = train_config['longer_gather_side']
            else:
                cfg[key] = _FALLBACK_MODEL_CFG[key]
                logging.warning(
                    f"train_config missing both 'seq_longer_gather_side' and '"
                    f"longer_gather_side', using fallback = {cfg[key]}")
            continue
